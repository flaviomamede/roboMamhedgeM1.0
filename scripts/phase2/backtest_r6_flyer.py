...
from roboMamhedgeR6 import run_backtest_trades
...
DEFAULT_OUTPUT_FILE = os.path.join(DEFAULT_OUTPUT_DIR, "flyer_r6.png")
...
def run_detailed_backtest(csv_path=DEFAULT_CSV_PATH):
    ...
    print("Simulando trades com roboMamhedgeR6.run_backtest_trades()...")
...
def generate_flyer(df, trades, equity, metrics, filename=DEFAULT_OUTPUT_FILE):
    ...
    ax1.set_title(f"Robot R6 Performance - {df.index[0].date()} to {df.index[-1].date()}", fontsize=16, fontweight="bold")
    ax1.grid(alpha=0.3)
