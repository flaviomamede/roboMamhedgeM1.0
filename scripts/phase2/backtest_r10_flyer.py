...
from roboMamhedgeR10 import run_backtest_trades
...
DEFAULT_OUTPUT_FILE = os.path.join(DEFAULT_OUTPUT_DIR, "flyer_r10.png")
...
def run_detailed_backtest(csv_path=DEFAULT_CSV_PATH):
    ...
    print("Simulando trades com roboMamhedgeR10.run_backtest_trades()...")
...
def generate_flyer(df, trades, equity, metrics, filename=DEFAULT_OUTPUT_FILE):
    ...
    ax1.set_title(f"Robot R10 Performance - {df.index[0].date()} to {df.index[-1].date()}", fontsize=16, fontweight="bold")
    ax1.grid(alpha=0.3)
    ...
    stats_text = (
        f"Net Profit: R$ {metrics['Net Profit']:,.2f} | ROI: {metrics['ROI (%)']:.2f}% | "
        f"Drawdown: {metrics['Max Drawdown (%)']:.2f}% | Trades: {metrics['Trades']} | "
        f"Win Rate: {metrics['Win Rate (%)']:.1f}% | Profit Factor: {metrics['Profit Factor']:.2f}"
