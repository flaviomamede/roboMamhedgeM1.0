"""
Flyer de desempenho do R10 v2 (Trend-Following + Efficiency Ratio + VWAP).
Consome roboMamhedgeR10v2.run_backtest_trades() diretamente com with_timestamps=True.
Saída: reports/phase2/flyer_r10v2.png
"""
import os
import sys
...
from roboMamhedgeR10v2 import run_backtest_trades
...
DEFAULT_OUTPUT_FILE = os.path.join(DEFAULT_OUTPUT_DIR, "flyer_r10v2.png")
...
def run_detailed_backtest(csv_path=DEFAULT_CSV_PATH):
    ...
    print("Simulando trades com roboMamhedgeR10v2.run_backtest_trades()...")
...
def generate_flyer(df, trades, equity, metrics, filename=DEFAULT_OUTPUT_FILE):
    print("Gerando gráfico flyer R10v2...")
    df_trades = pd.DataFrame(trades)
    ...
    ax1.set_title(
        f"Robot R10v2 — Trend + ER + VWAP | {df.index[0].date()} → {df.index[-1].date()}",
...
if __name__ == "__main__":
    ...
    print("\n--- Relatório de Performance R10v2 ---")
