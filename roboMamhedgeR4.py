"""
R4: MACD + RSI (long-only). RSI cruzando acima de 40.
"""
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    trades = run_backtest()
    if len(trades) == 0:
        print("[R4] Nenhum trade executado.")
    else:
        trades_r = np.array([pnl_reais(t) for t in trades])
        win_rate = (trades_r > 0).sum() / len(trades_r)
        print(f"[R4 - MACD+RSI] Trades: {len(trades_r)} ({N_COTAS} cotas) | Win: {win_rate*100:.1f}% | "
